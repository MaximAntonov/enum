...
function JAWS-ENUM {
	...
	$output = $output +  "Applocker Policy: " + ((Get-AppLockerPolicy -Effective | select -ExpandProperty RuleCollections) + "`r`n")
	$output = $output +  "Powershell Language Mode: " + (($ExecutionContext.SessionState.LanguageMode) + "`r`n")
